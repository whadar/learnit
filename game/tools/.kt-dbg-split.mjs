...
#endif`;
  const OUTF = `gl_FragColor = vec4( reflectedLight.directDiffuse * 2.0, 1.0 );
    ...
    for (const m of ms) { if (!m || seen.has(m) || !m.isMeshStandardMaterial) continue; seen.add(m); n++;
      ...
      m.onBeforeCompile = function (sh, r) { prev?.call(this, sh, r);
        sh.fragmentShader = sh.fragmentShader
          .replace('#include <lights_fragment_end>', END)
          .replace('#include <opaque_fragment>', OUTF); };
      m.needsUpdate = true; } });
  return 'patched ' + n;
}));
await settle(16);
await page.screenshot({ path: OUT+'dbg_split.png', timeout: 300000 });
await browser.close();
