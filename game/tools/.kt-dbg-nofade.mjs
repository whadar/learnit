...
console.log(await page.evaluate(() => {
  const S = window.__game.engine.scene, L = window.__game.engine.lighting;
  L.csm.fade = false;
  const seen = new Set(); let n=0;
  S.traverse(o => { if (!o.isMesh || !o.material) return; const ms = Array.isArray(o.material)?o.material:[o.material];
    for (const m of ms) { if (!m || seen.has(m) || !m.defines || !('CSM_FADE' in m.defines)) continue; seen.add(m); delete m.defines.CSM_FADE; m.needsUpdate = true; n++; } });
  return 'unfaded ' + n;
}));
await settle(14);
await page.screenshot({ path: OUT+'dbg_nofade.png', timeout: 300000 });
await browser.close();
