...
console.log(await page.evaluate(() => {
  const S = window.__game.engine.scene; let n=0, hits=0; const seen=new Set();
  S.traverse(o => { if (!o.isMesh || !o.material) return; const ms = Array.isArray(o.material)?o.material:[o.material];
    for (const m of ms) { if (!m || seen.has(m) || !m.isMeshStandardMaterial) continue; seen.add(m); n++;
      ...
      m.onBeforeCompile = function (sh, r) { prev?.call(this, sh, r);
        const before = sh.fragmentShader.length;
        sh.fragmentShader = sh.fragmentShader.split('( directLight.visible && receiveShadow )').join('( true )');
        if (sh.fragmentShader.length !== before) hits++;
      };
      m.needsUpdate = true; } });
  return 'patched ' + n;
}));
await settle(16);
await page.screenshot({ path: OUT+'dbg_force.png', timeout: 300000 });
await browser.close();
