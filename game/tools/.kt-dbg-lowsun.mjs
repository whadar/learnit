...
console.log(await page.evaluate(() => {
  const S = window.__game.engine.scene; let n=0;
  S.traverse(o => { if (o.isMesh && !o.receiveShadow) { o.receiveShadow = true; n++; } });
  const r = window.__game.setTime ? window.__game.setTime(17.3) : 'no setTime';
  window.__game.engine.renderer.shadowMap.needsUpdate = true;
  return 'recv+' + n + ' time ' + JSON.stringify(r);
}));
await settle(14);
await page.screenshot({ path: OUT+'dbg_lowsun.png', timeout: 300000 });
await browser.close();
