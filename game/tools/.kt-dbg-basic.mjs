...
console.log(await page.evaluate(() => {
  ...
  R.shadowMap.type = 0;            // BasicShadowMap: sampler2D + unpackRGBAToDepth
  R.shadowMap.needsUpdate = true;
  let n = 0; const seen = new Set();
  S.traverse(o => { if (!o.isMesh || !o.material) return; const ms = Array.isArray(o.material)?o.material:[o.material];
    for (const m of ms) { if (!m || seen.has(m)) continue; seen.add(m); m.needsUpdate = true; n++; } });
  return 'basic, materials ' + n;
}));
await settle(16);
await page.screenshot({ path: OUT+'dbg_basic.png', timeout: 300000 });
await browser.close();
