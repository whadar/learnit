...
console.log(await page.evaluate(() => {
  ...
  const glsl = `
  gl_FragColor = vec4( step( CSM_cascades[0].y, linearDepth ), step( CSM_cascades[1].y, linearDepth ), step( 0.999, linearDepth ), 1.0 );
  return;`;
...
await settle(14);
await page.screenshot({ path: OUT+'dbg_ld.png', timeout: 300000 });
await browser.close();
