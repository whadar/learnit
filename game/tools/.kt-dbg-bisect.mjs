...
console.log(await page.evaluate(() => {
  ...
  const glsl = `
  {
    ...
    float real = getShadow( directionalShadowMap[0], dls.shadowMapSize, dls.shadowIntensity, dls.shadowBias, dls.shadowRadius, vDirectionalShadowCoord[0] );
    float lit  = getShadow( directionalShadowMap[0], dls.shadowMapSize, 1.0, 0.0, 1.0, vDirectionalShadowCoord[0] );
    float rcv  = receiveShadow ? 1.0 : 0.0;
    gl_FragColor = vec4( real, lit, rcv, 1.0 );
    return;
  }`;
  const seen = new Set(); let n=0;
  S.traverse(o => { if (!o.isMesh || !o.material) return; const ms = Array.isArray(o.material)?o.material:[o.material];
    for (const m of ms) { if (!m || seen.has(m) || !m.defines || !('USE_CSM' in m.defines)) continue; seen.add(m); n++;
...
await settle(14);
await page.screenshot({ path: OUT+'dbg_bisect.png', timeout: 300000 });
await browser.close();
